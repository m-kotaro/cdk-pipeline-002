...
| AccountCode | アカウントコード (例: base, alfa, beta) | - |
...
export ENV=dev
export ACCOUNT_CODE=base
export REGION_NAME=tokyo

aws cloudformation deploy \
  --stack-name stack-${PROJECT_NAME}-${ENV}-${ACCOUNT_CODE}-${REGION_NAME}-pipeline-buckets \
...
Env, AccountCode, RegionName の値を変えるだけで別環境・別アカウント・別リージョン用のバケットセットが作成できる。
...
aws cloudformation deploy \
  --stack-name stack-${PROJECT_NAME}-dev-base-param \
  --template-file cfn/parameter-store.yaml \
  --parameter-overrides \
    ProjectName=${PROJECT_NAME} \
    Env=dev \
    AccountCode=base \
    PipelineAccountId=${ACCOUNT_ID} \
    TargetAccountId=${ACCOUNT_ID}
```

環境コードとアカウントコードを変えることで、任意の環境用パラメータを追加できる。
---
